...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function fetchAtletData() {
    try {
      const { data: athletes, error: errAthletes } = await supabase
        ...
        .eq('role', 'atlet') // <-- INI YANG BIKIN MEREKA MUNCUL BRAY!
      ...
      console.error(err);
      if (window.showToast) window.showToast('Gagal memuat data atlet', 'error');
    }
  }
  ...
  window.openEditClass = function(id, name, currentClass) {
    ...
    // Pilih option dropdown sesuai kelas yang aktif
  ...
  // PROSES SIMPAN KELAS KE DATABASE
  btnSaveClass.addEventListener('click', async () => {
    ...
    try {
      const { error } = await supabase
        .from('profiles')
        .update({ group_level: newClass })
        .eq('id', id);

      if (error) throw error;

      if (window.showToast) window.showToast('Berhasil memindahkan kelas!', 'success');
      
      // Update data lokal & render ulang tanpa reload halaman
